...
df_src = spark.sql("""
                   SELECT DISTINCT (CustomerID)as CustomerID, CustomerName, CustomerEmail, Country
                   FROM parquet.`abfss://silver@firstprojectstorageaccnt.dfs.core.windows.net/silverdata`
                   """)
df_src = df_src.dropDuplicates(['CustomerID'])
df_src.display()
...
path = 'abfss://gold@firstprojectstorageaccnt.dfs.core.windows.net/dim_Customer'

# if spark.catalog.tableExists("gold.dim_Customer"):
if DeltaTable.isDeltaTable(spark, path):
  df_sink = spark.sql(
          """
               select Dim_Customer_Key, CustomerID, CustomerName, CustomerEmail, Country from delta.`abfss://gold@firstprojectstorageaccnt.dfs.core.windows.net/dim_Customer`
           """
        )
  print('table exists')
...
# 1] in this we are getting schema from silver layer with 'where' condition to get blank schema table
  # df_sink = spark.sql(
  #         """
  #               select 1 as Dim_Customer_Key, CustomerID, CustomerName, CustomerEmail, Country from parquet.`abfss://silver@firstprojectstorageaccnt.dfs.core.windows.net/silverdata` where 1 = 0
  #         """
  #       )
...
if incremental_flag == '0':
    ...
    max_value = df_old.agg(max(df_old['Dim_Customer_Key'])).collect()[0][0]
max_value
...
df = df.withColumn('Dim_Customer_Key', max_value + monotonically_increasing_id())
df.display()
...
if DeltaTable.isDeltaTable(spark, path):
  ...
  deltaTable.alias('target').merge(df.alias('source'),'target.Dim_Customer_Key = source.Dim_Customer_Key').whenMatchedUpdateAll().whenNotMatchedInsertAll().execute()
else:
...
# MAGIC %sql
# MAGIC select * from delta.`abfss://gold@firstprojectstorageaccnt.dfs.core.windows.net/dim_Customer`
